...
async function createAsset(contract: Contract): Promise<void> {
    ...
    const base64Strings: string[] = readFileSync('4KB-CHUNK/100KB.txt', 'utf-8').split('\n');
    ...
    for (let i = 0; i < base64Strings.length; i++) {
        ...
        if (base64String.trim() !== '') {
            const assetId = i + 2000000; // Use the loop index as the assetId
            await contract.submitTransaction('SaveAsset', base64String, assetId.toString());
...
async function readAssetByID(contract: Contract): Promise<void> {
    ...
    const resultBytes = await contract.evaluateTransaction('QueryAsset', '50');
